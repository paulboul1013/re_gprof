...
**測試案例**:
- 多執行緒程式同時呼叫相同函數
- 驗證分執行緒模式下各執行緒的 call_count 正確
- 驗證合併模式下 call_count 為所有執行緒總和

**學習重點**:
- Thread ID 的取得 (`pthread_self()`, `gettid()`)
- 資料彙總演算法
- 命令列參數解析

**知識點更新**: 完成後在 `know.md` 新增「Phase 4: 多執行緒報表」章節
...
**目前位於**: Phase 3 (已完成) ✅
**下一步**: Phase 4 - 多執行緒報表輸出

**Phase 3 完成摘要**:
- ✅ 將核心變數改為 TLS (`__thread` 關鍵字)
- ✅ 實作 thread-safe 函數註冊機制 (mutex + global registry)
- ✅ 升級為 RUSAGE_THREAD 測量執行緒 CPU 時間
- ✅ 報表顯示 Thread ID (syscall(SYS_gettid))
- ✅ 建立 4 執行緒測試,驗證資料隔離
- ✅ 新增 `--multi-threaded` 命令列參數
- ✅ 詳細記錄學習成果於 `know.md` (含 TLS、pthread、mutex 等)
...
make                      # 編譯 (支援多執行緒)
./main                    # 單執行緒測試
./main --multi-threaded   # 多執行緒測試 (4 個執行緒)
```
