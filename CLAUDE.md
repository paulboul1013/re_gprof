...
**實作項目**:
- [x] 實作簡易 Hash Table 基礎結構 (Chaining)
- [x] Hash Table 基礎函數 (create, insert, find, free)
- [x] 核心函數重寫 (register_function, enter_function, leave_function)
- [x] PROFILE_FUNCTION 巨集更新 (使用函數名而非 ID)
- [x] profiling_handler 更新（使用函數名查找）
- [x] register_thread_data 重寫（深拷貝 hash table）
- [x] 所有報表函數重寫（遍歷 hash table）
- [x] export_dot 函數重寫（hash table 版本）
- [x] 全面測試與除錯
- [x] 修正 sys_time 計算 bug（ru_stime vs ru_utime）

**完成日期**: 2026-01-31

**實作細節**:
- ✅ Hash table 使用 chaining 解決碰撞，每個 bucket 是鏈表
- ✅ 三層 hash table 結構：
  - `hash_table_t`: 儲存函數資訊 (function_info_t)
  - `caller_hash_table_t`: 儲存 callee 呼叫次數
  - `caller_counts_hash_t`: 儲存 caller→callees 映射
- ✅ djb2 hash 函數：`hash = hash * 33 + c`
- ✅ 深拷貝機制：遍歷整個 hash table 並複製所有 entries
- ✅ 改為基於函數名而非 ID 的查找系統
- ✅ Call stack 改為儲存函數名字串（而非整數 ID）

**測試結果**:
- ✅ 單執行緒測試通過：正確顯示 CPU/IO/Sleep 時間
- ✅ 多執行緒測試通過：4 個執行緒獨立追蹤資料
- ✅ Per-thread 報表正常輸出
- ✅ Merged 報表正確彙總所有執行緒
- ✅ DOT 輸出正確生成 PNG 圖片

**Bug 修正**:
- ✅ sys_time 計算錯誤：第 528 行誤用 `ru_utime.tv_usec` 應為 `ru_stime.tv_usec`

**學習重點**:
- ✅ Hash function 設計 (djb2 字串哈希)
- ✅ Collision resolution (Chaining 鏈表法)
- ✅ 深拷貝 vs 淺拷貝（hash table 不能用 memcpy）
- ✅ 記憶體管理與動態分配 (malloc/free)
- ✅ 三層巢狀 hash table 設計
- ✅ 基於名稱 vs 基於 ID 的資料結構設計權衡

**知識點更新**: ✅ 將在 `know.md` 新增詳細的「Phase 5: Hash Table 實作」章節
...
**實作項目**:
- [x] 研究 gmon.out 二進位格式規格
- [x] 實作 gmon header 寫入 (magic "gmon" + version 1 + 12 bytes spare)
- [x] 實作 histogram (PC sampling) 資料寫入 (TAG 0x00,函數位址範圍對應 bins)
- [x] 實作 call graph arcs 寫入 (TAG 0x01,caller/callee 位址 + 呼叫次數)
- [x] 使用 `--export-gmon` 命令列選項觸發輸出

**測試案例**:
- [x] 產生 `gmon.out` 二進位檔案 (19KB)
- [x] 支援 merged 模式 (多執行緒合併) 與單執行緒模式
- ⚠️ 因缺乏真實 PC sampling 位址 (instrument-based 非 sampling-based),gprof 解析結果有限

**學習重點**:
- ✅ Binary file I/O in C (`fwrite`, `wb` 模式)
- ✅ gmon.out 檔案格式結構 (header + tagged records)
- ✅ Histogram bins 計算 (PC range → bin index)
- ✅ 函數位址取得 (`function_info_t.addr` 欄位)
- ✅ Little-endian 寫入 (native uint32_t/uint64_t 直接寫)

**重要發現**:
- gmon.out 使用 tagged record 格式：TAG 0 = time histogram，TAG 1 = call graph arc
- Header 固定 20 bytes：magic(4) + version(4) + spare(12)
- 本工具基於 instrumentation，self_time 來自 SIGPROF 採樣，histogram 為近似值
- `export_gmon_out()` 函數支援 merged/per-thread 兩種資料來源
...
**實作項目**:
- [x] 實作 ELF header 解析（Elf64_Ehdr, e_shoff, e_shstrndx）
- [x] 實作 section header table 解析（Elf64_Shdr, SHT_SYMTAB）
- [x] 讀取 `.symtab` 和 `.strtab` sections（透過 sh_link 關聯）
- [x] 將函數位址對應到符號名稱（elf_resolve_addr 二分搜尋）
- [x] 支援讀取外部 System.map 檔案（sysmap_load_symbols）

**新增功能**:
- `elf_load_symbols(path)`: mmap 讀取 ELF，解析 .symtab，回傳 sorted elf_sym_table_t
- `elf_resolve_addr(table, addr)`: 二分搜尋找最近函數符號
- `sysmap_load_symbols(path)`: 解析 System.map 格式文字檔
- `print_elf_symbol_report()`: 對比 profiler addr 與 ELF addr，顯示 OK/MISMATCH

**測試結果**:
- [x] 正常 binary: 66 個 ELF 符號，8/9 函數匹配（單執行緒）
- [x] 多執行緒: 11/11 函數匹配（merged hash table）
- [x] Stripped binary: 顯示正確錯誤訊息 "no .symtab found ... (stripped?)"
- [x] Stripped + System.map: 成功載入 66 個符號，8/9 匹配

**學習重點**:
- ✅ ELF (Executable and Linkable Format) 結構
- ✅ Symbol table 格式（Elf64_Sym, st_info, STT_FUNC）
- ✅ 位址到符號的對應（二分搜尋左邊界）
- ✅ `nm` 和 `objdump` 工具的原理
- ✅ mmap() 零拷貝讀取二進位檔案
- ✅ strip 對 .symtab 的影響
...
**知識點更新**: ✅ 已在 `know.md` 新增詳細的「Phase 7: ELF 符號解析」章節
...
**實作項目**:
- [x] 實作 DOT 格式輸出 (per-thread 和 merged 模式)
- [x] 使用顏色標示 hot path (高 self_time 的函數)
  - 紅色 (>20%): 非常熱
  - 橙色 (>10%): 熱
  - 黃色 (>5%): 溫暖
  - 淺綠 (>1%): 涼爽
...
**測試案例**:
- [x] 產生 `callgraph_merged.dot` (合併所有執行緒)
- [x] 產生 `callgraph_per_thread.dot` (分執行緒顯示)
- [x] 執行 `dot -Tpng callgraph_merged.dot -o callgraph_merged.png` ✅
- [x] 視覺化驗證 call graph 正確性 (function_io_heavy 顯示為紅色 97.9%)

**學習重點**:
- ✅ DOT 語言基礎 (digraph, node, edge, subgraph cluster)
- ✅ 圖形化資料視覺化 (顏色編碼、標籤格式)
- ✅ 整合外部工具 (Graphviz)
- ✅ Per-thread vs Merged 視覺化策略

**重要發現**:
- Per-thread 模式使用 `subgraph cluster` 將各執行緒分組
- 顏色編碼直觀顯示熱點函數 (function_io_heavy = 紅色)
- Merged 模式顯示 "N threads" 標籤,識別跨執行緒共享函數
- DOT 格式簡單易讀,適合手動編輯和除錯

**知識點更新**: ✅ 已在 `know.md` 新增詳細的「Phase 8: 視覺化輸出」章節
...
**Phase 7 完成摘要** (2026-02-20):
- ✅ 實作 ELF .symtab 解析（mmap + Elf64_Ehdr/Shdr/Sym）
- ✅ 實作二分搜尋 elf_resolve_addr()
- ✅ 實作 System.map 備援解析器
- ✅ 新增 print_elf_symbol_report() 對比報表
- ✅ 支援 --resolve-symbols、--resolve-symbols=PATH、--sysmap CLI 旗標
- ✅ 多執行緒模式使用 merged hash_table，11/11 匹配
- ✅ Stripped binary 顯示正確錯誤，System.map 備援驗證通過
...
./main --multi-threaded --export-dot              # 匯出 Call Graph (merged)
./main --multi-threaded --export-dot --dot-mode=per-thread  # 匯出 per-thread Call Graph
dot -Tpng callgraph_merged.dot -o callgraph.png  # 產生圖片
./main --resolve-symbols                          # ELF 符號解析（使用自身 binary）
./main --resolve-symbols=/path/to/binary          # 指定 ELF 檔案
./main --resolve-symbols=file --sysmap            # 使用 System.map 格式
```
